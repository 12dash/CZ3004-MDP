...
class Algorithm:
    ...
    def start_connection(self):
        ...
        try:
            ...
            print("[FAILED] Connection with Algorithm failed: " + str(error))
            return 0
    ...
    def read_from_client(self):
        try:
            ...
            if len(receive_msg) > 0:             
                print(f"[ALGORITHM] {receive_msg}")
                return receive_msg
            return None
        except Exception as error:
            print("[ERROR] Message can't be read from Algorithm")
            print("Algorithm failed to read: " + str(error))
            raise error
